...
prefix = 'section_';
SampleAddress = '20170704_D5_M2_NoOxyAgar';
...
parfor optical = 1:5
    ...
    SE = strel('disk',5); %% Define the growing way, disk shape and 5 pixel 
    cc1 = imdilate(cc, SE); %% Excute the pixel growing
    ...
    MeanIntens_red1 = [stats_red.MeanIntensity];
    PixelNum = sum(cc);
    PixelNum1 = sum(cc1);
    MeanRedB = (MeanIntens_red1*PixelNum1 - MeanIntens_red*PixelNum)/(PixelNum1 - PixelNum)
...
%            PE (GC > 0.04*RC | RC < 50) = 0; %% remove yellow dots
           ...
           for i = -10: 10
              for j = -10: 10
                 ...
                 if g > 6000 ||  r > 9000  %%To decide whether to write into the txt file. if with filter, g should > 8000
                    go = 0;
                 end
              end
           end
