...
prefix = 'section_';
SampleAddress = '20170305_D4_iv_GFPfil';
ResultAddress = 'G:/Jiagui/01_ABX_project/20170403_distribusion_theshold/';
...
for frame = 5:+20:100
 ...
 parfor optical = 1:5
    ...
    salm( redC < 200 | greenC > redC * 0.6667)  = 0; %%This is for GFP filter with PE, without red threshold
    ...
    c = struct2cell(s);
    stats_redC = regionprops( cc, redC, 'MeanIntensity');  %% for mCherry need to change
    MeanIntens_greenC = [stats_redC.MeanIntensity];
    ...
    stats_eccen = regionprops( cc, redC, 'Eccentricity');
    Eccentricity = [stats_eccen.Eccentricity];
    stats_eccen = regionprops( cc, redC, 'MajorAxisLength');
    MajorAxisLength = [stats_eccen.MajorAxisLength];
    stats_eccen = regionprops( cc, redC, 'MinorAxisLength');
    ...
    for component=1:cc.NumObjects
      ...
      if area > 5
         ...
         if x>800 && y>800 && x<(info.Width-800) && y<(info.Height-800)  %% crop bigger
           ...
           for i = -10: 10
              for j = -10: 10
                 ...
                 if g > 6000 ||  r > 6000 %%To decide whether to write into the txt file. if with filter, g should > 8000
                    go = 0;
                 end
              end
           end
         ...
         if go>0
             x_pos = x * 0.435;
             y_pos = y * 0.435;
             fprintf(fileID,'%10d %10d %10d %10.1f %10.1f %10.1f %4d %10.1f %10.1f %10.1f %10.1f %10.2f %10.1f %10.1f\n', frame, optical, component, x_pos, y_pos, z_pos, area, MeanIntens_red( component), MeanIntens_red( component), MeanIntens_green( component), MeanIntens_blue( component), MajorAxisLength( component) ,MinorAxisLength( component), Eccentricity( component));
