...
for frame = 1:100
   ...
   for optical = 1:5
     ...
     formatSpec = '%d %d %d %f %f %f %d %f %f %f %f %f %f %f';  %% '%10d %10d %10d %10.1f %10.1f %10.1f %4d %10.1f %10.1f %10.1f %10.1f %10.2f\n'
     sizeA = [14 Inf];
     if exist(fileName, 'file')
       ...
       A = A';
       fclose(fileID);
       number = number / 14;  %% count how many salm
       ...
       for i = 1:number
          ...
          salm( salmonella) = A( i, 7) * A(i, 8) / 1000;  %% What is cherry? 
...
% formatSpec = 'Frame Optical Component X Y Z Area Salm JoinedSalm Redmean Greenmean Bluemean Eccentricity';
...
for i = 1:salmonella
    if flag( i) < 1
       number = number + 1;
       x( number) = x( i); y( number) = y( i); z( number) = z( i);
       area( number) = area( i); salm( number) = salm( i); 
    end
end
salmonella = number;

 for i = 1:salmonella
     fprintf(fileID,'%10d %10d %10d %10.1f %10.1f %10.1f %10.1f %10.1f %10.1f %10d %10.1f %10.1f %10.1f %10.1f\n', frame(i), optical(i), Component(i), x(i), y(i), z(i), area(i), salm(i),Redmean(i), Greenmean(i), Bluemean(i), MajorAxisLength(i), MinorAxisLength(i), Eccentricity(i));
 end
