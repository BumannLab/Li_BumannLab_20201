...
prefix = 'section_';
SampleAddress = '20170704_D5_M2_NoOxyAgar4to8';
...
for optical = 1:5
    ...
    fileID = fopen(strcat('G:/Jiagui/01_ABX_project/20170704_distribusion_Timer\',SampleAddress,'/SalmTextPic/positions',counter, '_0', section, '.txt'),'w');
    ...
    salm( greenC < 350 | blueC > greenC * 0.95 | blueC < greenC * 0.5)  = 0; %|redC > greenC * 3
    ...
    SE1 = strel('disk',7);
    bw06 = imdilate(bw, SE);
    bw07 = imdilate(bw, SE1);
    bw08 = bw07-bw06;
    cc1 = bwconncomp(bw08);
    s1 = regionprops(cc1,'all'); %output the parameter of images. 'MajorAxisLength' and 'MinorAxisLength'
    c1 = struct2cell(s1);

    stats_red1 = regionprops( cc1, red, 'MeanIntensity');
    MeanIntens_redB = [stats_red1.MeanIntensity];
...
%         figure, imshow(imadjust(redIm),[])
      ...
      if area > 5
         ...
         go = 1;   
         if x>30 && y>30 && x<(info.Width-30) && y<(info.Height-30)  %% crop bigger
           RC = imcrop(red,[x-30 y-30 60 60]);
           GC = imcrop(green,[x-30 y-30 60 60]);
           BC = imcrop(blue,[x-30 y-30 60 60]);
           RC( 1, 1) = 1800; GC( 1, 1) = 2600; BC( 1,1) = 2200;
           
           imwrite(rgbImage,strcat('G:/Jiagui/01_ABX_project/20170704_distribusion_Timer\',SampleAddress,'/SalmTextPic/',counter, '_', section, '-', no,'.tif'));
           ...
           for i = -10: 10
              for j = -10: 10
                 ...
                 if g > 6000 ||  r > 10000  %%To decide whether to write into the txt file. if with filter, g should > 8000
                    go = 0;
                 end
              end
           end        
         ...
         if go>0
             x_pos = x * 0.435;
             y_pos = y * 0.435;
             fprintf(fileID,'%10d %10d %10d %10.1f %10.1f %10.1f %4d %10.1f %10.1f %10.1f %10.1f %10.2f %10.1f %10.1f %10f %10.1f\n', frame, optical, component, x_pos, y_pos, z_pos, area, MeanIntens_green( component), MeanIntens_red( component), MeanIntens_green( component), MeanIntens_blue( component), MajorAxisLength( component) ,MinorAxisLength( component), Eccentricity( component), MeanIntens_redB( component), RedVal(component));
